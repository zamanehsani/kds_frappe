...
const OrderDetailPanel: React.FC<OrderDetailPanelProps> = ({
  ...
  // Same status normalisation as OrderCard
  ...
  const notedItems = (order.items || [])
    .map((item, index) => ({
      ...
      qty: item.qty,
      note: item.notes?.replace(/<[^>]+>/g, " ").replace(/\s+/g, " ").trim(),
    }))
  ...
  // Payment summary — only if pricing data is available on this order
      ...
      <div className="relative w-full max-w-3xl max-h-[90vh] overflow-hidden rounded-[2rem] border border-olive-200 bg-white shadow-[0_24px_80px_rgba(15,23,42,0.22)]">
        <div className="flex items-center justify-between px-5 sm:px-7 py-4 sm:py-5 border-b border-olive-100 bg-white flex-shrink-0">
          <div className="flex items-center gap-3">
            ...
            <div>
              <div className="text-olive-900 font-black text-base sm:text-lg">KOT Details</div>
              <div className="text-olive-400 text-xs sm:text-sm">#{order.name.split("-").pop()}</div>
            </div>
          </div>
                  ...
                  {order.items?.map((item, i) => (
                    <li key={i}>
                      <button
                        ...
                        className={`w-full flex items-center gap-3 rounded-2xl border px-3 py-3 text-left transition-all duration-300 ${
                          completedItems.has(i)
                            ? "border-green-200 bg-green-50/80 shadow-sm scale-[0.99]"
                            : "border-olive-100 bg-white hover:border-olive-200 hover:shadow-sm"
                        } ${canMarkItemsDone ? "cursor-pointer" : "cursor-not-allowed opacity-80"}`}
                      >
                        <span
                          ...
                          {item.notes && (
                            <div
                              className={`text-xs sm:text-sm mt-1 line-clamp-2 transition-all duration-300 ${
                                completedItems.has(i) ? "text-olive-300" : "text-olive-400"
                              }`}
                              dangerouslySetInnerHTML={{
                                __html: item.notes.replace(/<[^>]+>/g, " ").trim(),
                              }}
                            />
                          )}
                        </div>
                ...
                (() => {
                  ...
                  return (
                    <button
                      ...
                      className={`w-full sm:w-auto rounded-xl flex items-center justify-center gap-2.5 px-4 sm:px-6 py-3 text-sm sm:text-base font-bold uppercase tracking-wider transition-all duration-200 active:scale-95 hover:shadow-md shadow-sm ${actionConfig.colors}`}
                    >
